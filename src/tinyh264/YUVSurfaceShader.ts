...
export default class YUVSurfaceShader {
  ...
  static _initShaderArgs (gl: WebGLRenderingContext, program: ShaderProgram) {
  ...
  static _initBuffers (gl: WebGLRenderingContext) {
    // Create vertex buffer object.
    return gl.createBuffer()
  }

  constructor (private gl: WebGLRenderingContext, private vertexBuffer: WebGLBuffer, private shaderArgs: ShaderArguments, private program: ShaderProgram) {
  }
   ...
   */
  updateShaderData (encodedFrameSize: {w:number, h:number}, h264RenderState: {maxXTexCoord:number, maxYTexCoord:number}) {
